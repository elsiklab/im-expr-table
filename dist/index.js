...
},{"./fetch-missing-data":6,"./has-fields":7}],6:[function(require,module,exports){
(function() {
  var buildProps, refs, refsIn, _;

  _ = require('underscore');

  buildProps = function(fs) {
    return function(vs) {
      if (vs) {
        return _.object(_.zip(fs, vs));
      } else {
        return {};
      }
    };
  ...
  refsIn = function(f) {
    ...
    for (i = _i = 1, _ref = parts.length; 1 <= _ref ? _i < _ref : _i > _ref; i = 1 <= _ref ? ++_i : --_i) {
      _results.push(parts.slice(0, i).join('.'));
    }
    return _results;
  };

  module.exports = function(type, fields, cache) {
    if (cache == null) {
      cache = {};
    }
    return function(service, id) {
      var key;
      key = service.root + '#' + id;
      return cache[key] != null ? cache[key] : cache[key] = (function() {
        return service.rows({
          select: fields,
          from: type,
          where: {
